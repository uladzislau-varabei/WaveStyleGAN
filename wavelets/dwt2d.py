# Thanks to: https://github.com/fbcotter/pytorch_wavelets/blob/master/pytorch_wavelets/dwt/transform2d.py

import torch
import torch.nn as nn
import pywt
from wavelets import dwt2d_utils
...
class DWTForward(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        # Prepare the filters
        filts = dwt2d_utils.prep_filt_afb2d(h0_col, h1_col, h0_row, h1_row)
    ...
    def forward(self, x):
        ...
        yh = []
        ll = x
        mode = dwt2d_utils.mode_to_int(self.mode)

        # Do a multilevel transform
        for j in range(self.J):
            # Do 1 level of the transform
            ll, high = dwt2d_utils.AFB2D.apply(
...
class DWTInverse(nn.Module):
    ...
    def __init__(self, wave='db1', mode='zero'):
        ...
        # Prepare the filters
        filts = dwt2d_utils.prep_filt_sfb2d(g0_col, g1_col, g0_row, g1_row)
...
class SWTForward(nn.Module):
    ...
    def forward(self, x):
        ...
        for j in range(self.J):
            # Do 1 level of the transform
            y = dwt2d_utils.afb2d_atrous(ll, filts, self.mode, 2**j)
            coeffs.append(y)
            ll = y[:,:,0]
