# Thanks to: https://github.com/fbcotter/pytorch_wavelets/blob/master/pytorch_wavelets/dwt/lowlevel.py
...
def afb1d(x, h0, h1, mode='zero', dim=-1):
    ...
    if mode == 'base':
        assert d in [2, 3]
        orig_size = x.shape[d]
        lohi = F.conv2d(x, h, padding='valid', stride=s, groups=C)
        upd_size = lohi.shape[d]
        num_pad = int((orig_size // 2 - upd_size) / 2)
        if d == 2:
            pad = (0, 0, num_pad, num_pad)
        elif d == 3:
            pad = (num_pad, num_pad)
        else:
            assert False
        lohi = F.pad(lohi, pad, mode='constant')
