...
from wavelets.utils import NCHW_FORMAT, NHWC_FORMAT, DEFAULT_DATA_FORMAT, COEFFS_SCALES_2D, \
    ...
    test_lifting_scheme, test_grad, find_scales
from vis_utils import prepare_input_image, show_lifting_results, prepare_all_images_paths
...
def fast_cdf97_2d_op(x, kernel, scale_1d_coeffs, scale_2d_coeffs, coeffs_scales_2d, data_format=DEFAULT_DATA_FORMAT):
    ...
    if scale_2d_coeffs:
        ...
        # Replace inplace to support 'cuda' bias_act
        x_LL *= coeffs_scales[0]
        x_LH *= coeffs_scales[1]
        x_HL *= coeffs_scales[2]
        x_HH *= coeffs_scales[3]
        x_LL = x_LL * coeffs_scales[0]
        x_LH = x_LH * coeffs_scales[1]
        x_HL = x_HL * coeffs_scales[2]
        x_HH = x_HH * coeffs_scales[3]
...
if __name__ == '__main__':
    ...
    # images_paths = prepare_all_images_paths(add_grayscale_paths=False)
    df = pd.read_csv('../dataset_csvs/FFHQ_v1.csv')
    images_paths = df['path'][:100]
    batch_size = 32
    scales = find_scales(images_paths, name='cdf-9/7', kernel=DEFAULT_KERNEL, forward_2d_op=fast_cdf97_2d_op,
