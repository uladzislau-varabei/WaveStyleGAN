...
def conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1, use_custom_op=True):
    if _should_use_custom_op(input) and use_custom_op:
        # print("Using gradfix conv2d")
        return _conv2d_gradfix(transpose=False, weight_shape=weight.shape, stride=stride, padding=padding, output_padding=0, dilation=dilation, groups=groups).apply(input, weight, bias)
    # print("Using ref conv2d")
    return torch.nn.functional.conv2d(input=input, weight=weight, bias=bias, stride=stride, padding=padding, dilation=dilation, groups=groups)

def conv_transpose2d(input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1, use_custom_op=True):
    if _should_use_custom_op(input) and use_custom_op:
        # print("Using gradfix transpose conv2d")
        return _conv2d_gradfix(transpose=True, weight_shape=weight.shape, stride=stride, padding=padding, output_padding=output_padding, groups=groups, dilation=dilation).apply(input, weight, bias)
    # print("Using ref transpose conv2d")
...
def _conv2d_gradfix(transpose, weight_shape, stride, padding, output_padding, dilation, groups):
    ...
    class Conv2d(torch.autograd.Function):
        ...
        def forward(ctx, input, weight, bias):
            ...
            ctx.save_for_backward(
                ...
                bias if bias is not None and bias.requires_grad else _null_tensor
            )
    ...
    class Conv2dGradWeight(torch.autograd.Function):
        # Thanks to: https://www.diffchecker.com/0rXiySXB/ (1)
        # Reduces memory usage: https://github.com/pytorch/pytorch/issues/74437, by jbschlosser (2)
        @staticmethod
        def forward(ctx, grad_output, input, bias):
            ...
            # Reduces memory usage but layout arg is not supported
            ...
            grad_weight = torch.ops.aten.convolution_backward(grad_output, input, empty_weight, bias_sizes=bias_shape,
                stride=stride, padding=padding, dilation=dilation, transposed=transpose,
                output_padding=output_padding, groups=groups, output_mask=[0, 1, 0])[1]
            assert grad_weight.shape == weight_shape
            return grad_weight
        ...
        def backward(ctx, grad2_grad_weight):
            ...
            # Additionally return None, to avoid error
            # function Conv2dGradWeightBackward returned an incorrect number of gradients (expected 3, got 2)
            # Thanks to https://github.com/pytorch/pytorch/issues/74437, by dave-epstein
