...
def prepare_all_images_paths(add_grayscale_paths=True):
    image_names = [
    ...
    images_paths = [os.path.join(path_prefix, 'wavelets', 'images', name) for name in image_names]
    return images_paths
...
def prepare_input_image(path_idx=0):
    ...
    path_prefix = os.path.dirname(os.path.realpath(__file__))
    image_path = os.path.join(path_prefix, 'wavelets', 'images', image_names[path_idx])
    max_size = 512
    if os.path.isfile(image_path):
        image = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)
    else:
        print(f'File {image_path} does not exist. Generating random data')
        image = np.random.uniform(0, 255, size=(max_size, max_size, 3)).astype(np.uint8)
...
def save_img(img, fname, use_pil=True):
    if use_pil:
